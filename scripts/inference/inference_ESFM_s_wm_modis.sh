...
#SBATCH --job-name=inference_ESFM_s_wm_modis         # short name for the job
...
#SBATCH --account=a122
#SBATCH --output=logs/inference_ESFM_s_wm_modis%j.out  # output log file
...
srun --ntasks=$nnodes \
  ...
  bash -c "
    ...
    torchrun \\
      ...
      inference.py \\
        --config ./configs/config_ESFM_s_wm_modis.yaml \\
        2>&1
  "
