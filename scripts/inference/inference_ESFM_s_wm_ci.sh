...
#SBATCH --job-name=inference_ESFM_s_wm_ci         # short name for the job
...
#SBATCH --account=a122
#SBATCH --output=logs/inference_ESFM_s_wm_ci%j.out  # output log file
...
srun --ntasks=$nnodes \
  ...
  bash -c "
    ...
    torchrun \\
      ...
      inference.py \\
        --config ./configs/config_ESFM_s_wm_ci.yaml \\
        2>&1
  "
