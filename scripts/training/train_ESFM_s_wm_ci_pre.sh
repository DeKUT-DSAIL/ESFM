...
#SBATCH --job-name=train_ESFM_s_wm_ci_pre          # short name for the job
...
#SBATCH --account=a122
#SBATCH --output=logs/train_ESFM_s_wm_ci_pre%j.out  # output log file
...
srun --ntasks=$nnodes \
  ...
  bash -c '
    ...
    torchrun \
      ...
      train.py \
        --config ./configs/config_ESFM_s_wm_ci_pre.yaml \
